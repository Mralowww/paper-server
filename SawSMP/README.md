...
- **戰績**：預設透過 PlaceholderAPI 讀取玩家原本就有的紀錄（擊殺、死亡、遊玩時間，佔位符可在 `stats.papi.placeholders` 自訂），定期同步到網站排行榜；另外自動記錄擊殺方式（末地水晶、重生錨、劍、斧頭、長矛、重錘、三叉戟、空手、弓箭、TNT、摔落）與連殺。沒有安裝 PAPI 或 `stats.source: plugin` 時改由插件自行統計。
