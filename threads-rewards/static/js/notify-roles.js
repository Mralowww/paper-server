...
(() => {
  ...
  let data = null; let busy = new Set();
  function render() {
    ...
    $("#nr-grid").innerHTML = d.roles.map((r, i) => `<button class="nr-btn ${r.on ? "on" : ""} ${busy.has(r.id) ? "busy" : ""}" data-id="${r.id}" style="--i:${i}" ${!d.logged_in || d.member === false ? "disabled" : ""}>
        <span class="nr-ic">${ART.icon(r.icon || "bell", 22)}</span>
        <span class="nr-txt"><b>${esc(r.name)}</b><small>${esc(r.desc || "")}</small></span>
        <span class="nr-state"><i class="nr-sw"><em></em></i><small>${t(r.on ? "nr.on" : "nr.off")}</small></span></button>`).join("");
  ...
  async function load() {
    data = await api(App.me ? "/api/notify-roles/me" : "/api/notify-roles", { quiet: true }); render();
  }
  document.addEventListener("app:ready", () => {
    ...
    $("#nr-grid").addEventListener("click", async (e) => {
      ...
      try { const res = await api(`/api/notify-roles/${r.id}`, { method: "POST", body: { on } }); r.count = res.count || r.count; App.ok(t(on ? "nr.added" : "nr.removed", { name: r.name }), "", 1200); }
      catch (err) { r.on = !on; r.count = Math.max(0, r.count + (on ? -1 : 1)); App.fail(err.message); }
